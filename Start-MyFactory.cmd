...
if not exist "backend\public\storage" (
  echo ربط مجلد التخزين public/storage...
  pushd backend && php artisan factory:ensure-storage-link && popd
)
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$api=$false;$web=$false;$healthy=$false;try{$t=New-Object Net.Sockets.TcpClient;$t.Connect('127.0.0.1',8000);$t.Close();$api=$true}catch{};try{$t=New-Object Net.Sockets.TcpClient;$t.Connect('127.0.0.1',3010);$t.Close();$web=$true}catch{};if($web){try{$r=Invoke-WebRequest -UseBasicParsing -Uri 'http://127.0.0.1:3010/myfactory/ar/login' -TimeoutSec 8;if($r.StatusCode -eq 200){$healthy=$true}}catch{}};if($api -and $web -and $healthy){exit 0};if($web -and -not $healthy){Get-NetTCPConnection -LocalPort 3010 -State Listen -ErrorAction SilentlyContinue ^| ForEach-Object { Stop-Process -Id $_.OwningProcess -Force -ErrorAction SilentlyContinue };if(Test-Path 'frontend\.next'){Remove-Item -Recurse -Force 'frontend\.next'}};exit 1"
if errorlevel 1 (
  ...
  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "$ok=$false;for($i=0;$i -lt 240;$i++){try{$a=New-Object Net.Sockets.TcpClient;$a.Connect('127.0.0.1',8000);$a.Close();$w=New-Object Net.Sockets.TcpClient;$w.Connect('127.0.0.1',3010);$w.Close();$r=Invoke-WebRequest -UseBasicParsing -Uri 'http://127.0.0.1:3010/myfactory/ar/login' -TimeoutSec 5;if($r.StatusCode -eq 200){$ok=$true;break}}catch{};Start-Sleep -Milliseconds 500};if(-not $ok){exit 1}"
  if errorlevel 1 (
    echo.
