...
import { productionApi, ProductionApiError, type QualityInspectionPhotoJson } from "@/lib/api/production-client";
import { resolveMediaUrl } from "@/lib/api/resolve-media-url";

type Props = {
  inspectionId: string;
  photos: QualityInspectionPhotoJson[];
  onChange: (photos: QualityInspectionPhotoJson[]) => void;
  disabled?: boolean;
};

function previewUrl(photo: QualityInspectionPhotoJson) {
...
export function QualityPhotoUploader({ inspectionId, photos, onChange, disabled }: Props) {
  ...
  const upload = useCallback(
    async (files: FileList | null) => {
      ...
      try {
        ...
        for (const file of Array.from(files)) {
          const res = await productionApi.uploadInspectionPhoto(inspectionId, file);
          next.push(res.data);
        }
        onChange(next);
      } catch (e) {
    ...
    [disabled, inspectionId, onChange, photos]
  );
  ...
  const replace = useCallback(
    async (photoId: string, files: FileList | null) => {
      ...
      setError(null);
      try {
        const res = await productionApi.replaceInspectionPhoto(photoId, file);
  ...
  const remove = async (photoId: string) => {
    ...
    setError(null);
    try {
      await productionApi.deleteInspectionPhoto(photoId);
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <p className="text-sm font-medium">صور الفحص</p>
        {!disabled ? (
