...
import { ChevronDown, ClipboardList, List, PlayCircle } from "lucide-react";

import { cn } from "@/lib/utils";

const PRODUCTION_BASE = "/ar/production";

const staticLinks = [
  { href: "/ar/production", label: "مركز التشغيل", icon: ClipboardList, exact: true },
  { href: "/ar/production/orders", label: "سجل الأوامر", icon: List, exact: false },
  { href: "/ar/production/operations", label: "تشغيل المشرف", icon: PlayCircle, exact: false }
] as const;
...
export function ProductionNavGroup({
  pathname,
  onNavigate
}: {
  pathname: string;
  ...
  const inSection = pathname.startsWith(PRODUCTION_BASE);
  ...
  return (
    <div className="grid gap-1">
      ...
      >
        ...
        <span className="flex-1 text-start">أوامر الإنتاج</span>
